...
在 Vim 中，你可以使用多种命令来打开文件，这些命令之间的主要区别在于它们如何处理当前的窗口和 buffer。

1. `:edit` 或 `:e`：这个命令用于在当前窗口中打开一个新的文件。如果当前的 buffer 已经被修改但还没有保存，Vim 会阻止你使用 :edit 命令，除非你添加了 ! 来强制执行命令（例如 :e! filename）。
2. `:split` 或 `:sp`：这个命令用于在一个新的水平窗口中打开文件。新的窗口会和当前的窗口共享同一份 buffer 列表。
3. `:vsplit` 或 `:vsp`：这个命令用于在一个新的垂直窗口中打开文件。新的窗口会和当前的窗口共享同一份 buffer 列表。
4. `:tabedit` 或 `:tabe`：这个命令用于在一个新的 tab 中打开文件。每个 tab 都有自己的窗口布局和 buffer 列表。
5. `:new`：这个命令用于在一个新的水平窗口中打开一个空的 buffer。你可以在这个新的 buffer 中创建一个新的文件。
6. `:vnew`：这个命令用于在一个新的垂直窗口中打开一个空的 buffer。你可以在这个新的 buffer 中创建一个新的文件。
7. `:enew`：这个命令用于在当前窗口中打开一个空的 buffer。你可以在这个新的 buffer 中创建一个新的文件。

这些命令都可以通过在命令后面添加文件名来打开特定的文件，例如 :edit filename。如果你不提供文件名，这些命令会打开一个新的空 buffer（除了 :edit 命令，如果不提供文件名，它会重新加载当前的 buffer）。

## 在 VIM 内执行外部命令的方法

输入 `:!` 然后紧接着输入一个外部命令可以执行该外部命令. 比如`:!ls`, `:!rm`
...
:s/foo/bar/g            " 要替换当前行中所有出现的搜索模式，请添加 g 标志
                        " 要忽略搜索模式的大小写，请使用i 标志
:#,#s/foo/bar/g         " 其中 #,# 代表的是替换操作的若干行中首尾两行的行号, 示例如下
...
:%s/^foo.*/Vim is the best/gc " c 表示在每次替换前确认，Vim 会询问你是否进行每个替换
```
...
" 普通模式
={motion}
==          " 自动对齐当前行, 这个命令会根据你的 indentexpr 或 cindent 设置来对齐当前行
gg=G        " 自动对齐整个文件。这个命令会将光标移动到文件的开始，然后对整个文件进行对齐。

>{motion}   " 例如 >10j 会增加从当前行开始的接下来 10 行的缩进
>>          " 缩进
...
> 请注意，这些命令的效果可能会受到你的 `tabstop`、`shiftwidth` 和 `expandtab` 设置的影响。你可以使用 `:set` 命令来查看或修改这些设置。例如，`set tabstop=4` 会将制表符的宽度设置为 4 个空格。

## 标签(mark)

```sh
m{a-zA-Z}   " 在当前位置做标签{a-zA-Z}
'{a-zA-Z}   " 移动到标签位置
...
- `zf`：创建一个新的折叠。你需要在 zf 后面添加一个移动命令来指定折叠的范围，例如 zf10j 会折叠从当前行开始的接下来 10 行。
- `zo`：打开一个折叠。你可以在 zo 后面添加一个数字来打开多个折叠，例如 3zo 会打开 3 个折叠。
- `zc`：关闭一个折叠。你可以在 zc 后面添加一个数字来关闭多个折叠，例如 3zc 会关闭 3 个折叠。
- `za`：切换一个折叠的状态。如果折叠是打开的，za 会关闭它；如果折叠是关闭的，za 会打开它。
- `zR`：打开所有的折叠。
- `zM`：关闭所有的折叠。

## 缩写

在 Vim 中，你可以使用 `:ab`（或 `:abbreviate`）h命令来创建文本缩写。当你在插入模式或命令行模式下输入一个缩写并按下空格或非字母数字字符时，Vim 会自动将缩写展开为完整的文本。

例如，如果你经常需要输入 `useCasperDialog`，你可以创建一个缩写来简化输入。在 Vim 的命令模式下，输入以下命令：

```sh
:ab ucd useCasperDialog
```

现在，每当你在插入模式下输入 `ucd` 并按下空格或非字母数字字符时，Vim 会自动将 `ucd` 替换为 `useCasperDialog`。

你可以使用 `:unab`（或 `:unabbreviate`）命令来删除一个缩写，例如：
```sh
:unab ucd
```
